...
build do
  ...
  # Ruby 3.1.x ext/openssl uses SSL_CTX_set_tmp_dh_callback() which is deprecated in
  # OpenSSL 3.0+ and carries the OSSL_DEPRECATEDIN_3_0 attribute. In OpenSSL 3.2.x that
  # attribute triggers a GCC ICE (internal compiler error: Segmentation fault) in the
  # MINGW toolchain on Windows. This patch guards the deprecated callback with an OpenSSL
  # version check and uses SSL_CTX_set_dh_auto() for OpenSSL >= 3.0.
  if windows? && version.satisfies?("~> 3.1")
