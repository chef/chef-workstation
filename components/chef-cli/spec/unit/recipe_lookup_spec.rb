require "spec_helper"
require "chef-cli/recipe_lookup"
require "chef/exceptions"
require "chef/cookbook/cookbook_version_loader"
require "chef/cookbook_version"
require "chef/cookbook_loader"

RSpec.describe ChefCLI::RecipeLookup do
  let(:repo_path) { "repo_path" }
  subject(:rp) { ChefCLI::RecipeLookup.new([repo_path]) }
    ...
    context "when a directory is provided" do
      ...
      context "the directory is not a cookbook" do
        it "raise an InvalidCookbook error" do
          ...
          expect { rp.load_cookbook(recipe_specifier) }.to raise_error(ChefCLI::RecipeLookup::InvalidCookbook)
        end
      end
    end
    ...
    context "when a cookbook name is provided" do
      ...
      context "and a cookbook does not exist" do
        it "raises an CookbookNotFound error" do
          ...
          expect { rp.load_cookbook(recipe_specifier) }.to raise_error(ChefCLI::RecipeLookup::CookbookNotFound)
        end
      end
    end
  end
  ...
  describe "#find_recipe" do
    ...
    context "no recipe is specified" do
      ...
      it "when there is no default recipe it raises a NoDefaultRecipe error" do
        ...
        expect { rp.find_recipe(cookbook_version) }.to raise_error(ChefCLI::RecipeLookup::NoDefaultRecipe)
      end
    end
    ...
    context "a recipe is specified" do
      ...
      it "when there is no recipe with that name it raises a RecipeNotFound error" do
        expect(cookbook_version).to receive(:recipe_filenames_by_name).and_return({})
        expect { rp.find_recipe(cookbook_version, desired_recipe) }.to raise_error(ChefCLI::RecipeLookup::RecipeNotFound)
      end
    end
  end
end
