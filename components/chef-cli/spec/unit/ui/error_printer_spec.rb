...
RSpec.describe ChefCLI::UI::ErrorPrinter do
  let(:orig_exception) { StandardError.new("test") }
  let(:target_host) { ChefCLI::TargetHost.instance_for_url("mock://localhost") }
  let(:wrapped_exception) { ChefCLI::WrappedError.new(orig_exception, target_host) }
  subject(:printer) { ChefCLI::UI::ErrorPrinter.new(wrapped_exception, nil) }
  ...
  context "#format_body" do
    RC = ChefCLI::TargetHost
    context "when exception is a ChefCLI::Error" do
  ...
  context ".show_error" do
    subject { ChefCLI::UI::ErrorPrinter }
    context "when handling a MultiJobFailure" do
      it "recognizes it and invokes capture_multiple_failures" do
        underlying_error = ChefCLI::MultiJobFailure.new([])
        error_to_process = ChefCLI::StandardErrorResolver.wrap_exception(underlying_error)
    ...
    context "when an error occurs in error handling" do
      it "processes the new failure with dump_unexpected_error" do
        ...
        error_to_process = ChefCLI::StandardErrorResolver.wrap_exception(StandardError.new("this is being shown"))
        # Intercept a known call to raise an error
        expect(ChefCLI::UI::Terminal).to receive(:output).and_raise error_to_raise
  ...
  context ".capture_multiple_failures" do
    ...
    before do
      allow(ChefCLI::Config).to receive(:error_output_path).and_return "/dev/null"
    ...
    it "should write a properly formatted error file" do
      ...
      job1 = double("Job", target_host: double("TargetHost", hostname: "host1"),
                           exception: ChefCLI::Error.new("CHEFUPL005"))
      ...
      multifailure = ChefCLI::MultiJobFailure.new([job1, job2] )
  ...
  context "#format_footer" do
    ...
    let(:show_stack) { true }
    let(:formatter) do
      ChefCLI::UI::ErrorPrinter.new(wrapped_exception, nil)
    end
  ...
  context ".write_backtrace" do
    let(:inst) { double(ChefCLI::UI::ErrorPrinter) }
    before do
      allow(ChefCLI::UI::ErrorPrinter).to receive(:new).and_return inst
    end
    ...
    it "formats and saves the backtrace" do
      ...
      ChefCLI::UI::ErrorPrinter.write_backtrace(wrapped_exception, orig_args)
    end
  end
end
