...
class ChefCLI::MockReporter
  def update(msg); ChefCLI::UI::Terminal.output msg; end

  def success(msg); ChefCLI::UI::Terminal.output "SUCCESS: #{msg}"; end

  def error(msg); ChefCLI::UI::Terminal.output "FAILURE: #{msg}"; end
end
...
def assert_string_lookup(key, retval = "testvalue")
  it "should look up string #{key}" do
    ...
    # guarantee ordering
    expect(ChefCLI::Text).to receive(top_level_method).
      and_return(tmock)
    call_seq.each do |m|
...
RSpec.configure do |config|
  ...
  config.before(:all) do
    ChefCLI::Log.setup "/dev/null", :error
    ChefCLI::UI::Terminal.init(File.open("/dev/null", "w"))
  end
end
