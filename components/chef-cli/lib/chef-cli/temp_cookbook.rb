  ...
  class TempCookbook
    ...
    def from_existing_recipe(existing_recipe_path)
      ...
      if cb
        ...
        ChefCLI::Log.debug("Found full cookbook at path '#{cb[:path]}' and using recipe '#{cb[:recipe_name]}'")
        name = cb[:name]
        ...
        # in the future
        ChefCLI::Log.debug("Found single recipe at path '#{existing_recipe_path}'")
    ...
    def from_resource(resource_type, resource_name, properties)
      ...
      ChefCLI::Log.debug("Generating cookbook for single resource '#{resource_type}[#{resource_name}]'")
    ...
    class UnsupportedExtension < ChefCLI::ErrorNoLogs
