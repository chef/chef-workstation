...
text = ChefCLI::Text.commands

ChefCLI.commands do |c|
  # TODO must be a better API we can do than `top_level` and `create`
  c.top_level("target", :Target, text.target, "chef-cli/command/target", subcommands: [
    c.create("converge", [:Target, :Converge], text.target.converge, "chef-cli/command/target/converge", cmd_alias: "converge" ),
  ])

  c.top_level("config", :Config, text.config, "chef-cli/command/config", subcommands: [
    c.create("show", [:Config, :Show], text.config.show, "chef-cli/command/config/show"),
  ])
  ...
  c.top_level("version", :Version, text.version, "chef-cli/command/version")
  ...
  c.top_level("hidden-root", :Base, text.base, "chef-cli/command/base",
