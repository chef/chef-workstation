...
module ChefCLI::UI
  class ErrorPrinter
    ...
    def self.show_error(e)
      ...
      unwrapped = ChefCLI::StandardErrorResolver.unwrap_exception(e)
      if unwrapped.class == ChefCLI::MultiJobFailure
    ...
    def self.capture_multiple_failures(e)
      out_file = ChefCLI::Config.error_output_path
      e.params << out_file # Tell the operator where to find this info
      File.open(out_file, "w") do |out|
        e.jobs.each do |j|
          wrapped = ChefCLI::StandardErrorResolver.wrap_exception(j.exception, j.target_host)
    ...
    def format_body
      if exception.kind_of? ChefCLI::Error
    ...
    def format_footer
      if show_log
        if show_stack
          t.footer.both(ChefCLI::Config.log.location,
                        ChefCLI::Config.stack_trace_path)
        else
          t.footer.log_only(ChefCLI::Config.log.location)
        end
      else
        if show_stack
          t.footer.stack_only
    ...
    def save_backtrace(output)
      File.open(ChefCLI::Config.stack_trace_path, "ab+") do |f|
        f.write(output.string)
      end
    end

    def self.error_summary(e)
      if e.kind_of? ChefCLI::Error
        # By convention, all of our defined messages have a short summary on the first line.
        ChefCLI::Text.errors.send(e.id, *e.params).split("\n").first
