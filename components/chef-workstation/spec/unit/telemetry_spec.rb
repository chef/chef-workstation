...
RSpec.describe ChefWorkstation::Telemetry do
  subject(:telemetry) { ChefWorkstation::Telemetry }
  let(:dev_mode) { true }
  let(:config) { double("config") }

  before do
    allow(config).to receive(:dev).and_return dev_mode
    allow(ChefWorkstation::Config).to receive(:telemetry).and_return config
  ...
  context "#make_event_payload" do
    context "when event is ':run'" do
      it "adds expected properties" do
        ...
        expect(props[:version]).to eq ChefWorkstation::VERSION
